#!/bin/bash
#SBATCH --job-name=pretrain_savgol_d1
#SBATCH --partition=dgx2,dgx
#SBATCH --gres=gpu:1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=10
#SBATCH --mem=50G
#SBATCH --output=/mnt/homeGPU/igarzon/Meta-Learning/ijepa_spectra2d/logs/gadf_soil_nir_savgol_d1/slurm_%j.out
#SBATCH --error=/mnt/homeGPU/igarzon/Meta-Learning/ijepa_spectra2d/logs/gadf_soil_nir_savgol_d1/slurm_%j.err
...
# ── Directorio de logs ────────────────────────────────────────────────────────
mkdir -p logs/gadf_soil_nir_savgol_d1

# ── Info del job ──────────────────────────────────────────────────────────────
echo "=========================================="
echo "Job ID:   $SLURM_JOB_ID"
echo "Node:     $SLURMD_NODENAME"
echo "Config:   configs/gadf_soil_nir_savgol_d1.yaml"
echo "GPU:      cuda:0"
echo "=========================================="

# ── Pretraining ───────────────────────────────────────────────────────────────
RUN_DIR="logs/gadf_soil_nir_savgol_d1/gadf_jepa_savgol_d1_${SLURM_JOB_ID}"
BEST_CKPT="${RUN_DIR}/gadf_jepa_savgol_d1-best_probe.pth.tar"

python main.py \
    --fname configs/gadf_soil_nir_savgol_d1.yaml \
    --devices cuda:0 \
    --epochs 200 \
    --run_id "${SLURM_JOB_ID}" \
    --wandb_name "ijepa_savgol_d1_ep200"
...
SAVE="results/region_gadf2d_savgol_d1/ft_gadf_jepa_savgol_d1_${SLURM_JOB_ID}_ftlr1e-4/"
