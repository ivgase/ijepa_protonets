#!/bin/bash
#SBATCH --job-name=pretrain_joint_savgol
#SBATCH --partition=dgx2
#SBATCH --gres=gpu:1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=10
#SBATCH --mem=80G
#SBATCH --output=/mnt/homeGPU/igarzon/Meta-Learning/ijepa_spectra2d/logs/gadf_soil_nir_joint_savgol/slurm_%j.out
#SBATCH --error=/mnt/homeGPU/igarzon/Meta-Learning/ijepa_spectra2d/logs/gadf_soil_nir_joint_savgol/slurm_%j.err
...
# ── Rutas ─────────────────────────────────────────────────────────────────────
# RUN_ID fijo → carpeta predecible, sin colisiones entre jobs en paralelo
RUN_ID="ep800"
TAG="gadf_joint_savgol"
BASE_LOG="$(pwd)/logs/gadf_soil_nir_joint_savgol"
...
echo "Config:   configs/gadf_soil_nir_joint_savgol.yaml"
...
python main_joint_pretrain.py \
    --fname configs/gadf_soil_nir_joint_savgol.yaml \
    --devices cuda:0 \
    --run_id "${RUN_ID}" \
    --epochs 400 \
    --wandb_name "joint_savgol_ep400"
...
python main_joint_pretrain.py \
    ...
    --final_lr 5e-7 \
    --warmup 10 \
    --wandb_name "joint_savgol_ep800_lowlr"

echo ">>> ETAPA 3 completada. Entrenamiento savgol finalizado."
